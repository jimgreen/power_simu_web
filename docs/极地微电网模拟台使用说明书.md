...
- 绿电功率按照“直流负荷功率 + 交流负荷功率 - 柴发功率”计算；绿电占比按照“绿电功率 / (直流负荷功率 + 交流负荷功率) * 100%”计算。
...
面向外部系统的设备、遥测、遥信、遥调和遥控数组接口，以及模型版本一致性校验规则，详见 [模拟台外部 WEB 接口](external-web-api.md)。
...
$telemetry.time
$telemetry.values."Environment.weather.WIND_SPEED"
...
Invoke-RestMethod -Method POST -Uri "http://127.0.0.1:8710/api/external/telemetry/query?model_id=simple_model" `
  -ContentType "application/json" `
  -Body '{"yc_names":["Environment.weather.WIND_SPEED","Environment.weather.SOLAR_IRRADIANCE"],"yx_names":["ACGenerator.diesel_300kw.run_stat"]}'
```
...
while ($true) {
  ...
  $wind = $yc.values."Environment.weather.WIND_SPEED"
  $pEss = 0
