...
class RenewableControlPlannerDataQualityTest(unittest.TestCase):
    ...
    def test_diesel_below_guard_holds_renewable_when_storage_is_discharging(self):
        snapshot = renewable_snapshot()
        for row in snapshot["measurements"]["scada"]:
            if row["dev_name"] == "storage-1" and row["meas_type"] == "P_GEN":
                row["value"] = 10
        ...
        self.assertEqual(
            plan["metrics"]["renewableControlAction"],
            "hold_low_diesel_no_charge",
        )
        self.assertEqual(plan["metrics"]["storageControlAction"], "hold")

    def test_diesel_below_guard_curtails_renewable_when_storage_is_charging(self):
        snapshot = renewable_snapshot()
        for row in snapshot["measurements"]["scada"]:
            if row["dev_name"] == "storage-1" and row["meas_type"] == "P_GEN":
                row["value"] = -5
    ...
    def test_diesel_below_guard_holds_acdc_and_renewable(self):
        snapshot = renewable_snapshot()
    ...
    def test_low_diesel_does_not_request_reverse_power_when_converter_is_idle(self):
        ...
        self.assertAlmostEqual(
            plan["metrics"]["renewableTarget"],
            plan["metrics"]["renewableCurrentKw"],
        )
        self.assertTrue(plan["metrics"]["renewableRaiseBlockedByDieselGuard"])
    ...
    def test_low_diesel_reduces_converter_injection_while_renewable_holds(self):
        snapshot = renewable_snapshot()
        for row in snapshot["measurements"]["scada"]:
            if row["dev_name"] == "diesel-1" and row["meas_type"] == "P_GEN":
                row["value"] = 10
    ...
    def test_task8_metrics_use_topology_side_and_capacity_weighted_soc(self):
        ...
        self.assertAlmostEqual(metrics["acStorageCurrentKw"], 4.0)
        self.assertAlmostEqual(metrics["dcStorageCurrentKw"], -12.0)
...
class RenewableControlBackendApiTest(unittest.TestCase):
    ...
    def test_task8_trend_point_copies_side_totals_and_dc_transfer_groups(self):
        ...
        expected_breakdown = {
            ...
            "acStorageTargetKw": plan["metrics"]["acStorageTargetKw"],
            "dcStorageCurrentKw": -12.0,
            "dcStorageTargetKw": plan["metrics"]["dcStorageTargetKw"],
            "totalStorageCurrentKw": -8.0,
            "totalStorageTargetKw": plan["metrics"]["totalStorageTargetKw"],
